...
class ArticleDetailController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //获取随机4条文章
        $random_article = $articleModel::where('article_show', 1)->where('nav_id', $nav_id)->whereRaw("id >= (select floor(rand() * (select max(id) from `blog_nav_article`)))")->take(4)->get();
        //获取热门文章
        $hot_article = $articleModel::where('article_show', 1)->where('nav_id', $nav_id)->orderBy('article_click', 'desc')->take(6)->get();

        return view('home.article_details.index', compact('random_article','hot_article','article_result', 'article_url', 'badge_arr', 'previousPostID', 'nextPostID', 'article_message', 'bg_arr'));
    }
